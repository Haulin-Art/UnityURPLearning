    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes input)
            {
                ...
                float4 state = SAMPLE_TEXTURE2D_LOD(_HeightTex, sampler_HeightTex, input.uv, 0);
                float2 velocity = state.rg;
                float height = state.b;

                float3 positionOS = input.positionOS.xyz;
                //positionOS.y += (height - 1.0) * _WaveHeight;
            ...
            {
                float4 state = SAMPLE_TEXTURE2D(_HeightTex, sampler_HeightTex, input.uv);
                float2 velocity = state.rg;
                float height = state.b;
                ...
                float h_right = SAMPLE_TEXTURE2D(_HeightTex, sampler_HeightTex, input.uv + float2(texelSize.x, 0)).b;
                float h_left = SAMPLE_TEXTURE2D(_HeightTex, sampler_HeightTex, input.uv - float2(texelSize.x, 0)).b;
                float h_up = SAMPLE_TEXTURE2D(_HeightTex, sampler_HeightTex, input.uv + float2(0, texelSize.y)).b;
                float h_down = SAMPLE_TEXTURE2D(_HeightTex, sampler_HeightTex, input.uv - float2(0, texelSize.y)).b;
                ...
                float depthFactor = saturate((height - 0.5) * 0.5);
