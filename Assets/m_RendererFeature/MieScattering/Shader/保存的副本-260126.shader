    ...
    SubShader
    {
        ...
        float3 mAtomsScattering(atmosData m)
        {
            ...
            for (int i=0 ; i < m.numSamples ; i++)
            {
                ...
                //earthBlock = inter2.x>0 && inter2.y>0 && inter2.y>inter2.x ? earthBlock : true;
                while (lstep < m.numLigSamples && !earthBlock)
                {
                    // 当前光学深度采样点处的海拔高度
                    float lh = length(lp - m.planetCenter) - m.planetRadius;
                    // 预判下一个点的海拔，要是在海拔下，就停止
                ...
                // ligDep = D（P，C）+ D（P，A）
                // 我想问的就是这里，这里正不正确，因为不加这里的话，感觉天空的颜色不对，更加偏向于蓝白
                // 但是加了之后，天际线始终偏黄橙色，并且在日出的时候好像不正确
        ...
        float4 frag(Varyings i) : SV_Target
        {
            ...
            float3 planetCenter = float3(ro.x, ro.y-_PlanetRadius-_Altitude*1e3, ro.z);

            // 计算与大气层的交点
            float2 atmosphereIntersect = RaySphereIntersect(ro, rd, planetCenter, _PlanetRadius + _AtmosphereHeight);
            
            // 如果没有与大气层相交，返回原始颜色
            if (atmosphereIntersect.x < 0.0 && atmosphereIntersect.y < 0.0)
                return originalColor;
            
            // 计算射线长度
            float rayLength = atmosphereIntersect.x>0 && atmosphereIntersect.y>0 && 
                atmosphereIntersect.y > atmosphereIntersect.x 
                ? atmosphereIntersect.x:atmosphereIntersect.y;
            
            // 如果深度不是天空盒，则计算到几何表面的距离
            if (depth < 1.0)
            {
                // 重建世界位置
                float3 worldPos = ReconstructWorldPosition(i.uv, depth);
                float surfaceDist = length(worldPos - ro);
                
                // 如果几何表面在大气层内，则使用表面距离
                if (surfaceDist < atmosphereIntersect.y)
                {
                    rayLength = surfaceDist;
                }
            }
            // 确保射线长度有效
            if (rayLength <= 0.0)
            ...
            //float3 scattering = CalculateSingleScattering(ro, rd, rayLength, planetCenter, normalize(_SunDirection));
            float3 scattering = AtomsScattering(ro, rd, rayLength, planetCenter, normalize(_SunDirection));
            float sun = dot(rd, normalize(_SunDirection));
            sun = 3.0*smoothstep(0.9985,0.9988,sun) + 1.0;
            

            float3 jiaodain = ro + rayLength*rd;
            float2 jd =  RaySphereIntersect(jiaodain, -normalize(_SunDirection), planetCenter, _PlanetRadius+5000);
            float earthBlock = jd.x < 0.0 && jd.y < 0 ? 0.0 : abs(jd.y-jd.x);
            earthBlock = smoothstep(0.0,0.5,(jd.y - jd.x)*2.0/(_PlanetRadius ));
            earthBlock = pow(earthBlock,1.5);
            //float3 scattering = CalculateAtmosphericScattering(ro, rd, rayLength, planetCenter, -normalize(_SunDirection));
            
            float scaleFactor = 100000;
            float atmosH = _AtmosphereHeight/scaleFactor;
            float planetR = _PlanetRadius/scaleFactor;

            float3 newPlanetCenter = float3(0.0,- planetR,0.0);
            // 计算光线与大气层的交点
            ...
            float yvzhi = step(0.9999,Linear01Depth(depth,_ZBufferParams));
            ...
            // 计算视角弦中点，计算采样数
            // 原理就是大气层面向摄像机的中心切面，然后计算这个切面的dis
            float edgeFactor = length(ro+rd*(inter.x+rayLen*0.5));
            edgeFactor = 1.0 - smoothstep(planetR, planetR + atmosH , edgeFactor);
            edgeFactor = pow(edgeFactor,1.0);
            edgeFactor = 1.0;
            float samCount = ceil(lerp(32,16,edgeFactor));
            ...
            data.rayleighScattering = 1*scaleFactor*float3(0.000058, 0.000135, 0.00033);// 瑞利散射系数
            data.mieScattering = 1*scaleFactor*float3(0.00002,0.00002,0.00002); // 米尔散射系数
            ...
            float yuanchu = step(Linear01Depth(depth,_ZBufferParams),0.001);
            originalColor.xyz *= yuanchu;

            float3 finalCol;

            finalCol = scatter*edgeFactor*5.0*step(0.0,rayLen) + originalColor;
