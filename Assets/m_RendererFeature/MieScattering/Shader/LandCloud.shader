Shader "Unlit/LandCloud"
{
    ...
    SubShader
    {
        ...
        Pass{
            ...
            // 常数定义
            #define NUM_STEPS 32       // Ray Marching步数，影响细节和性能
            #define SHADOW_STEPS 6      // 阴影计算步数
            ...
            float CalculateLight(float3 pos, CloudParams params) {
                ...
                float shadow = 1.0;
                float shadowStepSize = rayLen/(SHADOW_STEPS*10.0); // 阴影步长，根据场景调整
                float3 shadowPos = pos + params.sunDir * shadowStepSize; // 避免自相交
                for (int i = 0; i < SHADOW_STEPS; i++) {
                    float density = GetDensity(shadowPos)*100.0;

                    shadow *= BeerPowderLaw(EXTINCTION,density,3.0);
            ...
            float4 frag(Varyings i) : SV_Target
            {
                ...
                float3 ro = _WorldSpaceCameraPos; // 直接使用相机世界位置
                ...
                // 光线步进测试
                float3 planetCenter = TransformObjectToWorld(float3(0,0,0));
                ...
                float rayLen = sphereDis.y - sphereDis.x;

                float3 rayStart = ro + rd * sphereDis.x;

                Light ld =GetMainLight();
                float3 liDir = ld.direction;

                CloudParams cd;
                cd.sunDir= liDir;     // 归一化的太阳方向
                cd.ro = ro ;         // 摄像机起点（世界坐标）
                cd.rd = rd;
                ...
                return float4(cloud.rgb*10, saturate(cloud.a*30.0));
            }
            ENDHLSL
        }
    }
