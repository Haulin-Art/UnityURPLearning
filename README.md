...
基于浅水方程（Shallow Water Equations）的实时水面波纹模拟系统，通过 Compute Shader 实现高性能 GPU 计算。
...
| --------------- | ----------------- | ------ |
| dt              | 时间步长              | 0.016  |
| advectSpeed     | 平流速度              | 1.0    |
| damping         | 衰减系数              | 0.01   |
| pressureStrength | 压力强度（高度梯度对速度的影响） | 0.5    |
| restoreStrength | 恢复力强度             | 0.1    |
| sourceRadius    | 源半径               | 0.05   |
...
2. **UV 岛跳跃**：创新性地实现跨 UV 岛的无缝流体传输
3. **重力方向图**：支持复杂曲面上的重力驱动流体
4. **配套工具链**：完整的重力图和邻接图生成工具
5. **GPU 加速**：全 Compute Shader 实现，实时性能
6. **调试友好**：丰富的调试模式，便于问题排查
