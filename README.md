...
> 物理真实的大气散射效果，支持从大气层内到太空中的完整视角，提供后处理、LUT 预计算、物体材质三种实现方式，并集成体积云系统。
...
基于 `AdvancedVolumetricCloud.shader` 的体积云渲染系统，与大气散射系统深度集成，实现真实的云层效果。

#### 技术实现

**核心组件**

- **AdvancedVolumetricCloud.shader**：体积云渲染核心着色器
- **云层密度场**：程序化生成云层密度
- **光照计算**：考虑大气散射对云层颜色的影响
...
实现大气散射与体积云的深度融合，大气颜色会影响体积云的颜色，形成统一的天空环境。
...
5. **体积云集成**：深度融合体积云系统，实现真实天空效果
6. **大气-云层联动**：大气颜色影响体积云，形成统一的天空环境

***

## 水面渲染系统

> 仿 FluidFlux2 的真实水体渲染系统，包含完整的海浪动画、SDF 距离场、焦散效果、水体散射模糊、次表面散射等特性，并与大气散射系统深度集成。

***

![水面渲染系统](./Assets/ShowImages/WaterRender/Water_Foam.png)
![水面渲染系统](./Assets/ShowImages/WaterRender/Water_Scattering_02.png)
![水面渲染系统](./Assets/ShowImages/WaterRender/Water_Scattering_01.png)

### 水渲染与大气散射联合

#### 核心功能

实现水体与大气散射系统的深度融合，大气颜色影响水体外观，光照影响水的丁达尔阴影和焦散效果。
...
![水面渲染系统](./Assets/ShowImages/WaterRender/Water_waterLine_01.png)
![水面渲染系统](./Assets/ShowImages/WaterRender/Water_waterline_Tindal.png)

#### 核心功能

基于 `WaterLine` 系统的吃水线和水下效果，实现物体与水面的真实交互。

#### 技术实现

**核心组件**

- **WaterLineRF.cs**：Renderer Feature 实现，处理吃水线效果
- **control_WaterLineRenderer.cs**：吃水线渲染控制
- **cess.cs**：辅助脚本
- **ScreenWSPos.shader**：屏幕空间世界位置计算
...
6. **大气-水体联动**：大气散射系统与水体渲染深度集成
7. **吃水线系统**：实现物体与水面的真实交互效果
8. **丁达尔阴影**：受大气光照影响的体积光效果
...
![表面流体系统](./Assets/ShowImages/SurfaceFluid/fluid3.png)
...
核心思想：在 UV 岛的边缘外围区域（描边区域），存储邻接 UV 岛对应位置的数据，实现跨岛数据传递。

```hlsl
// 1. 采样 UV 岛邻接图
float4 jump_factor = UVJumpMap.SampleLevel(samplerLinearClamp, uv, 0);
// jump_factor.xy = 邻接目标 UV
// jump_factor.z  = 边缘遮罩
// jump_factor.a  = UV 岛范围

// 2. 计算跳跃遮罩（jump_mask = 1 表示在边缘外围区域）
...
当流体从一个 UV 岛流到另一个 UV 岛时，速度场在 UV 空间中的方向可能与实际曲面上的流动方向不一致。通过将速度方向重映射到重力方向，确保流体继续沿曲面正确流动。
...
重力图必须设置描边外扩，使其范围完整包括 UV 岛邻接图的范围，否则边缘外围区域无法获取正确的重力方向。
...
| 通道 | 内容 | 说明 |
|-----|------|-----|
| R | 邻接 UV.x | 邻接位置的目标 UV 坐标 |
| G | 邻接 UV.y | 邻接位置的目标 UV 坐标 |
| B | 边缘遮罩 | 是否为 UV 岛边缘（0 或 1） |
| A | UV 岛范围 | UV 岛内部区域（边缘内为 1，外围为 0） |
...
```
         UV 岛邻接图                    切向空间重力图        床底高度图
    ┌──────────────┐              ┌──────────────┐   ┌──────────────┐
    │ RG:邻接UV    │              │ RG:重力方向   │   │ R:地形高度   │
    │ B:边缘遮罩   │              │ (切向空间)    │   │ (噪波纹理)   │
    │ A:UV岛范围   │              └──────┬───────┘   └──────┬───────┘
    └──────┬───────┘                     │                  │
           │                             │                  │
...
| v1.6 | 支持 RenderTexture 作为 UV 跳跃图输入 |
